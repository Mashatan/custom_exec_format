...
struct custom_hdr{
	...
	unsigned long 	certificate_table;  /* File pointer to digital certificate table	*/
...
struct certificate_table{
	char 			type_cert;	/* 1-ED25519, 2-RAS, ... */
	unsigned char   sign[32];	/* Signature 256bit,
								for signing   => sign(Data (HMAC-SHA256) + copyright, privateKey) ==> DataSigned , 
...
struct address{
	unsigned short			arch_type;		/* Set CPU architectures							*/
	unsigned long long		entryptr;		/* Entry point  (RVA)								*/
